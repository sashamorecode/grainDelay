...
if %1 == 1 (echo Making grainDelay Windows DEMO VERSION distribution ...) else (echo Making grainDelay Windows FULL VERSION distribution ...)
...
REM - msbuild grainDelay-app.vcxproj /p:configuration=release /p:platform=win32

echo Building 32 bit binaries...
msbuild grainDelay.sln /p:configuration=release /p:platform=win32 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly 

echo Building 64 bit binaries...
msbuild grainDelay.sln /p:configuration=release /p:platform=x64 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly;append
...
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\grainDelay.aaxplugin\Contents\Win32\grainDelay.aaxplugin --out .\build-win\aax\bin\grainDelay.aaxplugin\Contents\Win32\grainDelay.aaxplugin
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\grainDelay.aaxplugin\Contents\x64\grainDelay.aaxplugin --out .\build-win\aax\bin\grainDelay.aaxplugin\Contents\x64\grainDelay.aaxplugin
...
:32-Bit-is
"%ProgramFiles%\Inno Setup 5\iscc" /Q /cc ".\installer\grainDelay.iss"
goto END-is

:64-Bit-is
"%ProgramFiles(x86)%\Inno Setup 5\iscc" /Q /cc ".\installer\grainDelay.iss"
goto END-is

:END-is

REM - Codesign Installer for Windows 8+
REM -"C:\Program Files (x86)\Microsoft SDKs\Windows\v7.1A\Bin\signtool.exe" sign /f "XXXXX.p12" /p XXXXX /d "grainDelay Installer" ".\installer\grainDelay Installer.exe"

REM -if %1 == 1 (
REM -copy ".\installer\grainDelay Installer.exe" ".\installer\grainDelay Demo Installer.exe"
REM -del ".\installer\grainDelay Installer.exe"
REM -)
