...
def  main():
  ...
  installer = "\installer\grainDelay Installer.exe"
   
  if demo:
    installer = "\installer\grainDelay Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\grainDelay manual.pdf" 
  ]
  ...
  ZIPNAME = "grainDelay-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "grainDelay-v" + FULLVERSIONSTR + "-win-demo.zip"
